...
The database schema is bundled through `schema.js` so the first serverless invocation does not depend on a filesystem `schema.sql` path that may not exist inside the deployed function bundle.
