...
app.get("/api/export.xlsx", requireAuth, async (req, res) => {
  ...
  const expenses = (await q(`
    ...
    FROM expenses
    WHERE user_id=$1
    ORDER BY expense_date, expense_time
  `, [uid])).rows;

  const credits = (await q(`
    SELECT credit_date AS "Date",
           source AS "Source",
           amount AS "Amount",
           description AS "Description"
    FROM credits
  ...
  const monthly = (await q(`
    SELECT TO_CHAR(DATE_TRUNC('month',expense_date),'YYYY-MM') AS "Month",
           COALESCE(SUM(amount),0) AS "Total Expenses",
  ...
  for (const [name, data] of sheets) {
    ...
    if (data.length > 0) {
      ...
      worksheet.columns = columns.map(key => ({
        header: key,
        key: key,
        width: Math.max(14, Math.min(32, key.length + 4))
      }));
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="expense-tracker-${dateString()}.xlsx"`
  );
